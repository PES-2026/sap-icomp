...
export class RescheduleSchedule {
  constructor(
  ...
  async execute(dto: RescheduleScheduleDTO): Promise<Result<void, ApplicationError>> {
    const scheduleResult = await this.scheduleRepository.findById(dto.scheduleId);

    if (!scheduleResult) {
      return Result.fail(new ScheduleNotFoundError(dto.scheduleId));
    }
    ...
    // Release old slots
    await this.scheduleSlotRepository.releaseSlotsByScheduleId(dto.scheduleId);
    ...
    const courseName = ""; // TODO: Get the course value when it's guest;
